...
case UCHAR :
	{
		...
		*/
		...
		int minx, maxx, miny, maxy, point1, point2, point3, point4,j,k,flag1,flag2;
		...
		for(i = 1; i<=comp_count; i++)
			...
			{
				hor_allign[i-1][j-1] = abs(ccattarray[i]->maxy-ccattarray[j]->maxy);
		...
		for(i = 1; i<=comp_count; i++)
			for(j=1;j<=comp_count;j++)
			{
				...
				ver_allign[i-1][j-1] = abs((ccattarray[j] -> minx+((ccattarray[j]->maxx-ccattarray[j]->minx)/2)) - (ccattarray[i] -> minx+((ccattarray[i]->maxx-ccattarray[i]->minx)/2)));
			...
			for(j=i+1;j<=comp_count;j++)
			{
				if(hor_allign[i-1][j-1] <= 5)
					hor_allign[i-1][j-1] = 1;
				else hor_allign[i-1][j-1] = 0;
				if(ver_allign[i-1][j-1] <= 10)
			...
			/*for(i=0; i< comp_count; i++)
				for(j=i+1;j<comp_count;j++)
					if(hor_allign[i][j] == 1){
						if(ver_allign[i][j] != 1){
							for(k=j;k<comp_count;k++)


							hor_allign[i][j] = count;
							k=j;
						}
						else if(ver_allign[k][j] == 1){
							hor_allign[i][j] = count;
							k=j;
						}
						else { hor_allign[i][j]=0;	 
					}
			*/			
		...
		for(i = 1; i<=comp_count; i++){
			...
			for(j=1;j<=comp_count;j++)
			{
				if(hor_allign[i-1][j-1]!=0)fprintf(fp,"%d:%d,",j,hor_allign[i-1][j-1] );
		...
		for(i = 1; i<=components[0].pointsAboveHighThreshold; i++)
			for(j = i+1; j<= components[0].pointsAboveHighThreshold; j++)
			{
				if((ccattarray[i] -> minx+((ccattarray[i]->maxx-ccattarray[i]->minx)/2)) > (ccattarray[j] -> minx+((ccattarray[j]->maxx-ccattarray[j]->minx)/2)))
				{
					temp = ccattarray[i]->minx;
		...
		for (i = 1; i<=components[0].pointsAboveHighThreshold; i++){
			for (j = i+1; j<= components[0].pointsAboveHighThreshold; j++ )
			{
				if (ccattarray[i] != NULL && ccattarray[j] != NULL )
				{
					if ((abs(ccattarray[i] -> maxy - ccattarray[j]  -> maxy) <= 10))
					{	
						attlink = ccattarray[i];
						while(attlink->next) attlink = attlink->next;
						attlink -> next = ccattarray[j];
		...
		//int *text[components[0].pointsAboveHighThreshold+1];
		//for(i = 0; i<=components[0].pointsAboveHighThreshold; i++)
		//	text[i] = NULL;
		//int *entry;
		...
		{
			attlink = ccattarray[i];
			if(!attlink) printf("Null");
			else
			while(attlink)
			{
				printf("label=%d", attlink->textlabel);
		...
		{
			//printf("\n");			
			if(ccattarray[i] && ccattarray[i]->next)
			{
				//printf("\nhere%d ",i);
				//ccattarray[i] = listsort(ccattarray[i]);
				attlink = ccattarray[i];
				flag1 = 1;
				while(attlink && attlink -> next)
				{
					//printf("here\n");
					//printf("sum:%d\t Diff:%d\n", ((attlink->maxx - attlink->minx)+(attlink->next->maxx - attlink->next->minx)), (attlink->next->minx - attlink->maxx) );
					if(abs((attlink->maxx - attlink->minx)+(attlink->next->maxx - attlink->next->minx))>= 2*(abs(attlink->next->minx - attlink->maxx)))
					{	
						flag1 = 1;
						
						attlink ->textlabel = attlink -> next-> textlabel = j;
												
						//printf("%d,%d,%d,%d;",attlink ->miny,attlink ->maxy,attlink ->minx,attlink ->maxx);
						//j++;
						//printf("%d\n",j);
						/*for(k = 0; k<=v; k++)
							if ((k%theDim[0] <= attlink -> maxx && k%theDim[0] >= attlink -> minx && k/theDim[0] <= attlink -> maxy && k/theDim[0] >= attlink -> miny) || (k%theDim[0] <= attlink -> next -> maxx && k%theDim[0] >= attlink -> next -> minx && k/theDim[0] <= attlink -> next -> maxy && k/theDim[0] >= attlink -> next -> miny) )
								theBuftemp[k] = 255;*/
					}
					else
						flag1 = 0;
			...
			if(attlink){				
				while(attlink){			
					if(attlink -> textlabel>0){					
						//textlink = ccattarraytext[attlink -> textlabel];
						textlink = attlink;
						attlink1 = attlink;
						attlink2 = attlink;						
						int label1 = textlink->textlabel;
						printf("in:%d\n", textlink->textlabel);
						while(textlink) if(textlink -> textlabel == label1){printf("inner\n"); textlink = textlink -> next; attlink1 = attlink;attlink=attlink->next;}
						else
						{	textlink = NULL; attlink1->next = NULL;}
						ccattarraytext[attlink2 -> textlabel] = attlink2;
						//while(textlink) textlink = textlink -> next;
						//textlink = attlink;	
						//attlink = attlink->next;
					}
					else
		...
		for(i = 0; i<=components[0].pointsAboveHighThreshold; i++)
		{
			...
			while(attlink)
			{
				if(minx>attlink->minx) minx = attlink->minx;
				if(maxx<attlink->maxx) maxx = attlink->maxx;
				if(miny>attlink->miny) miny = attlink->miny;
				if(maxy<attlink->maxy) maxy = attlink->maxy;
				attlink = attlink->next;
			...
			while(attlink)
			{
				if(attlink -> textlabel > 0)
				{
					/*for (k=attlink ->minx;k<=attlink ->maxx; k++){
						theBuftemp[attlink ->miny*theDim[0]+k] = 255;
						theBuftemp[(attlink ->miny-1)*theDim[0]+k] = 255;
						theBuftemp[(attlink ->miny-2)*theDim[0]+k] = 255;
						theBuftemp[(attlink ->miny-3)*theDim[0]+k] = 255;
						theBuftemp[attlink ->maxy*theDim[0]+k] = 255;
						theBuftemp[(attlink ->maxy+1)*theDim[0]+k] = 255;
						theBuftemp[(attlink ->maxy+2)*theDim[0]+k] = 255;
						theBuftemp[(attlink ->maxy+3)*theDim[0]+k] = 255;
				
					}
					for (k=attlink ->miny;k<=attlink ->maxy; k++){
						theBuftemp[k*theDim[0]+attlink ->minx] = 255;
						theBuftemp[k*theDim[0]+attlink ->minx-1] = 255;
						theBuftemp[k*theDim[0]+attlink ->minx-2] = 255;
						theBuftemp[k*theDim[0]+attlink ->minx-3] = 255;
						theBuftemp[k*theDim[0]+attlink ->maxx] = 255;
						theBuftemp[k*theDim[0]+attlink ->maxx+1] = 255;
						theBuftemp[k*theDim[0]+attlink ->maxx+2] = 255;
						theBuftemp[k*theDim[0]+attlink ->maxx+3] = 255;
		...
		/*for(i = 0; i<=components[0].pointsAboveHighThreshold; i++)
		{
			...
			while(attlink)
			{
				if(attlink -> next)
				{
					if(((!attlink->textlabel) && (attlink->next->textlabel)) || (attlink->textlabel))
					attlink = attlink->next;
					else
					{
						attlink = 
		...
		//legend(ccattarray,components[0].pointsAboveHighThreshold,&minx, &maxx, &miny, &maxy,theDim[0], inputBuf );
		printf("\nminx=%d, maxx=%d, miny=%d, maxy = %d\n", minx, maxx, miny, maxy);
		if(minx!=10000 && maxx!=0)
		{
			/*for (k=minx;k<=maxx; k++){
				theBuftemp[miny*theDim[0]+k] = 255;
				theBuftemp[(miny-1)*theDim[0]+k] = 255;
		...
		for(k=0;k<v;k++)
			if(k/theDim[0] <=maxy && k/theDim[0] >=miny && k%theDim[0]>=minx && k% theDim[0]<=maxx );
			else 
				//theBuftemp[k] = 0;
		...
		//	if(k/theDim[0]==105)
		//		theBuftemp[k] = 255;
		//legend(ccattarray, &minx, &maxx, &miny, &maxy);
		...
		/*for(i = 1; i<=components[0].pointsAboveHighThreshold; i++)
		{
			...
			while(attlink)
			{
				printf("%d,%d,%d,%d;",attlink ->label,attlink ->textlabel,attlink ->minx,attlink ->maxx);
				attlink = attlink -> next;
		...
		*/
		...
		//for ( j=0; j<v; j++, theBuf++ ) { 
		//		printf("%d",*theBuf);
		//	}
		//i = 1;int j = 1;
		// printf("\n%d\n",(u8)components[0].pointsAboveHighThreshold);
		/* while(components[i].label<=components[0].pointsAboveHighThreshold+1) { //assuming the labels are sorted in the component structure.
		if((u8)components[i].pointsAboveHighThreshold>0 && (u8)components[i].pointsAboveLowThreshold>0)		
		printf ("i = %d\n label = %d\t pointsAboveLowThreshold = %d\t pointsAboveHighThreshold = %d\n",i,(u8)components[i].label,(u8)components[i].pointsAboveLowThreshold,(u8)components[i].pointsAboveHighThreshold);
		i++;
		}*/
