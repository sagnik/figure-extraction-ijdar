...
case UCHAR :
	{
		...
		*/
		...
		for (i = 1,j=1; i<=components[0].pointsAboveHighThreshold; i++){
			...
			fprintf(fp,"area of %d th component: %d\npercentage area of the component:%f\n", i,area,percent_area);
			if(area>10 && percent_area<10.0)
			{
		...
		for(i = 1; i<=components[0].pointsAboveHighThreshold; i++)
			...
			{
				if(ccattarray[i] -> minx > ccattarray[j] -> minx)
				{
		...
		for (i = 1; i<=components[0].pointsAboveHighThreshold; i++){
			for (j = i+1; j<= components[0].pointsAboveHighThreshold; j++ )
			{
				...
				{
					if ((abs(ccattarray[i] -> maxy - ccattarray[j]  -> maxy) <= 4))
					{	
		...
		for (i = 1, j=1; i<=components[0].pointsAboveHighThreshold; i++)
		{
			...
			if(ccattarray[i] && ccattarray[i]->next)
			{
				...
				while(attlink && attlink -> next)
				{
					...
					if((attlink->next->minx > attlink->maxx) && ((attlink->maxx - attlink->minx)+(attlink->next->maxx - attlink->next->minx))>=  (attlink->next->minx - attlink->maxx))
					{	
						flag1 = 1;
