...
*/
		...
		int minx, maxx, miny, maxy, point1, point2, point3, point4,j;
		...
		/*for (i = 1; i<=components[0].pointsAboveHighThreshold; i++){
			for (j = i; j<= components[0].pointsAboveHighThreshold; j++ )
			{
				if ((abs(ccattarray[i] -> maxy - ccattarray[j]  -> maxy) <= 3) && ((ccattarray[i] -> maxx - ccattarray[i] -> minx)))
