...
static int CheckAndEvaluateConnectivity( int connectivity, int dimz );
...
case UCHAR :
	{
		...
		*/
		...
		fprintf(fp,"\nv=%d; comps=%d; %d,%d\n",v,components[0].pointsAboveHighThreshold,theDim[0],theDim[1]);
		//get rectangular boxes around the components
		int minx, maxx, miny, maxy, point1, point2, point3, point4,j,k,ll,flag,flag1,flag2;
		for (i = 1,j=1; i<=components[0].pointsAboveHighThreshold; i++){
			if(ccarray[i]!=NULL){
				...
				//printf("area of i=%dth component: %d\npercentage area of the component:%f,length=%d,height=%d\n", i,area,percent_area,ll,k);
				...
				if((percent_area<4.0) && abs(maxx - minx) < ((0.1) * theDim[0]) && abs(maxy - miny)<((0.1) * theDim[1]))// comment it for axis
				{
					printf("area of i=%d, j=%d th component: %d\npercentage area of the component:%f\n", i,j,area,percent_area);
		...
		long strings[comp_count][comp_count];
		long stringstat[comp_count][comp_count];
		...
		for(i = 1; i<= comp_count; i++)
			for(j = i+1; j<= comp_count; j++)
			{
				...
				if(ccattarray[i] -> maxx > ccattarray[j] -> maxx)
				{
		...
		for(i = 1; i<=comp_count; i++)
			for(j=1;j<=comp_count;j++)
			{
				...
				ver_allign[i-1][j-1] = ccattarray[j] -> maxx - ccattarray[i] -> maxx;
			...
			/*int mindistance;
			for(i=1;i<comp_count;i++){ k=i; mindistance = theDim[0];
				for(j=i+1;j<comp_count;j++)
					if((mindistance > ccattarray[j] -> maxx - ccattarray[k] -> maxx) && (ccattarray[j] -> minx - ccattarray[k] -> minx > 0) && (hor_allign[i-1][j-1]==1)){
						mindistance = ccattarray[j] -> maxx - ccattarray[k] -> maxx;
						k=j;
					}
					//else
					//	k=j;
			...
			/*for(i=1;i<comp_count;i++){ k=i;
				for(j=i+1;j<comp_count;j++)
					if( (hor_allign[i-1][j-1] > 0)){ //printf("%d\t",ccattarray[j] -> maxx - ccattarray[k]->maxx);
						if(ccattarray[j] -> maxx - ccattarray[k]->maxx > .05*theDim[0])
		...
		long l,m,n;		
		k=0;l=0;
		for(i=0;i<comp_count;i++){m=i; l=0;
			for(j=i+1;j<comp_count;j++)
				if(hor_allign[i][j]>0){
						printf("%d,%d\t",m,(ccattarray[m+1]->maxx - ccattarray[j+1]->minx));		
					if(abs(ccattarray[m+1]->maxx - ccattarray[j+1]->minx)<10){
						strings[k][l] = m+1;
						printf("there\t");						
						strings[k][l+1] = j+1;
						l=l+2;
		...
		k=0;	
		for(i = 0; i<comp_count; i++){
			//printf("Horiz for conponent %d: ",i);
			//flag = 0;			
			for(j=0;j<comp_count;j++)
			{
				if(strings[i][j]>0){
					labelarray[k] = ccattarray[strings[i][j]]->label;					
					k++;
		...
		/*for(i = 0; i<comp_count; i++){
			...
			{
				if(strings[i][j]>0){
					if(stringstat[i][0]>ccattarray[strings[i][j]]->minx) 
						stringstat[i][0]=ccattarray[strings[i][j]]->minx;
					if(stringstat[i][1]<ccattarray[strings[i][j]]->maxx) 
						stringstat[i][1]=ccattarray[strings[i][j]]->maxx;
					if(stringstat[i][2]>ccattarray[strings[i][j]]->miny) 
						stringstat[i][2]=ccattarray[strings[i][j]]->miny;
					if(stringstat[i][3]<ccattarray[strings[i][j]]->maxy) 
						stringstat[i][3]=ccattarray[strings[i][j]]->maxy;
		...
		k=0;
		for(i=0;i<comp_count;i++) if(strings[i][0]>0){		
		for (k=stringstat[i][0];k<=stringstat[i][1]; k++){
						theBuftemp[stringstat[i][2]*theDim[0]+k] = 255;
						theBuftemp[(stringstat[i][2]-1)*theDim[0]+k] = 255;
						//theBuftemp[(miny-2)*theDim[0]+k] = 255;
						theBuftemp[stringstat[i][3]*theDim[0]+k] = 255;
						theBuftemp[(stringstat[i][3]+1)*theDim[0]+k] = 255;
						//theBuftemp[(maxy+2)*theDim[0]+k] = 255;
						//theBuftemp[(maxy+3)*theDim[0]+k] = 255;
					}
					for (k=stringstat[i][2];k<=stringstat[i][3]; k++){
						theBuftemp[k*theDim[0]+stringstat[i][0]] = 255;
						theBuftemp[k*theDim[0]+stringstat[i][0]-1] = 255;
						//theBuftemp[k*theDim[0]+minx-2] = 255;
						//theBuftemp[k*theDim[0]+minx-3] = 255;
						theBuftemp[k*theDim[0]+stringstat[i][1]] = 255;
						theBuftemp[k*theDim[0]+stringstat[i][1]+1] = 255;
						//theBuftemp[k*theDim[0]+maxx+2] = 255;
						//theBuftemp[k*theDim[0]+maxx+3] = 255;
					}
		}*/
		for(i=0;i<comp_count;i++){ printf("\nstring %d:", i);
			for(j=0;j<comp_count;j++);
		...
		//for(i=0;i<theDim[1];i++)
			//theBuftemp[2+i*theDim[0]] = 255;//verticle line
		for(i=0;i<comp_count;i++)
