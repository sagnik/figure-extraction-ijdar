...
main(int argc, char *argv[])
{
  ...
  if(argc!=3)
    {
      printf("Usage: flip_pgm image-in image-out\n");
      exit(0);
    }
  printf("%d\n",argc);
  ...
  for(i=0;i<rows*cols;i++)
    {
      image[i]=MAX_GRAY-image[i];
    }

  printf("flipping finished\n");

  if((write_pgm_image(outfilename,image,rows,cols,NULL,MAX_GRAY))!=1)
    {
      printf("Writing image-out error\n");
      exit(0);
    }
