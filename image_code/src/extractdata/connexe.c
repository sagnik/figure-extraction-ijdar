...
void legend(ccAttList* ptrlist[], int length,int* minx, int* maxx, int* miny, int* maxy);
...
case UCHAR :
	{
		...
		*/
		...
		for (i = 1; i<=components[0].pointsAboveHighThreshold; i++){
			...
			ccattarray[i] -> label = ccarray[i] -> label;
			ccattarray[i] -> minx = minx;
			ccattarray[i] -> maxx = maxx;
			ccattarray[i] -> miny = miny;
			ccattarray[i] -> maxy = maxy;
			ccattarray[i] -> textlabel = 0;
			ccattarray[i] -> next = NULL;
			printf("textlabel:%d\n",i );
		...
		/*legend(ccattarray,components[0].pointsAboveHighThreshold,&minx, &maxx, &miny, &maxy );
		printf("\nminx=%d, maxx=%d, miny=%d, maxy = %d\n", minx, maxx, miny, maxy);
		for(k=0;k<v;k++)
			if(k/theDim[0] <=maxy+20 && k/theDim[0] >=miny-5 && k%theDim[0]>=minx-50 && k% theDim[0]<=maxx+5 );
			else 
				theBuftemp[k] = 0;
		*/		
...
void legend(ccAttList* ptrlist[], int length,int* minx, int* maxx, int* miny, int* maxy)
{
	int i,j,found_case1,found_case2, containstext=0,diffy=0, diffx=0;
	...
	for(i = 0; i<length;i++)
	{
		attlink = ptrlist[i];		
		while(attlink)
			{//printf("%d,%d,%d,%d\n;",ptrlist[i] ->miny,ptrlist[i]->maxy,ptrlist[i]->minx,ptrlist[i] ->maxx);attlink = attlink->next;
}
	...
	for(i = 0; i<containstext-1;i++)
	{
		...
		{	diffy= textarray[i+1]->miny - textarray[i]->miny; }			
